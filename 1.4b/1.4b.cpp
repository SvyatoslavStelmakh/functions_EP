...
int main()
{
    ...
    std::cout << std::setw(setw) << FindRootA(Function1, k_iter, 4, 1, eps) << std::setw(setw) << Function1(FindRootA(Function1, k_iter, 4, 1, eps)) << std::setw(setw) << k_iter << std::endl;

    std::cout << std::setw(setw) << "s" << std::setw(setw) << "x" << std::setw(setw) << "f(x)" << std::setw(setw) << "k_iter" << std::endl;
    for (double s = 0.95; s <= 1.21; s += 0.05)
    {
        std::cout << std::setw(setw) << s << std::setw(setw) << FindRootB(Function2, k_iter, s, 1.5, 0, eps) << std::setw(setw) << Function2(FindRootB(Function2, k_iter, s, 1.5, 0, eps), s) << std::setw(setw) << k_iter << std::endl;
    }
}